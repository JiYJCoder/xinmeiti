...
function sy_sns(){
	if(!$config)$config = F('Config');
	$sns_user = $config['sns_user'];
	$sns_password = $config['sns_password'];
	$xmldata = file_get_contents("http://www.dxton.com/webservice/sms.asmx/GetNum?account=$sns_user&password=$sns_password");
	$data = (array)simplexml_load_string($xmldata);
	$data = $data[0]*10;
	return $data;
}
//发送短信类
function send_sns($mobile,$content){
	if(!$config)$config = F('Config');
	$sns_user = $config['sns_user'];
	$sns_password = $config['sns_password'];
	$content = urlencode($content);
	$r = file_get_contents("http://sms.106jiekou.com/utf8/sms.aspx?account=$sns_user&password=$sns_password&mobile=$mobile&content=$content");
	return $r;
}
//短信报错
function sns_error($r){
	$array = array(
		'100' => '发送成功',
		'101' => '验证失败',
		'102' => '手机号码格式不正确',
