...
class TagLibYp extends TagLib
{
	protected $tags   =  array(
		...
		'type'=>array('attr'=>'id,key,parentid','level'=>3),//gzse7en 2014
	...
	public function _flash($attr,$content) {
		...
		$count_flash = M('Slide_data')->where("fid = $flashid")->count();
	...
	public function _block($attr,$content) {
		...
		$type   = !empty($tag['type'])? $tag['type'] : '';//gzse7en增加
		$mod    = isset($tag['mod'])?$tag['mod']:'2';
	...
	public function _subcat($attr,$content) {
		...
		$catid = !empty($tag['catid'])? $tag['catid'] : '0';
		$num = !empty($tag['limit'])?$tag['limit']:'99';//自定义调出栏目数量,可以利用排序把不需要出来的放在后面，仅调出前面需要的几个_by gzse7en
		...
		$parsestr .= 'if($'.$key.'<'.$num.') :';
		$parsestr .= 'if('.$ifleft.' intval('.$catid.')==$'.$id.'["parentid"] '.$condition.$selfcondition.' ) :';
		$parsestr .= '++$'.$key.';?>';
		$parsestr .= $content;
		$parsestr .= '<?php endif;?>';
		$parsestr .= '<?php endif;?>';
	...
	public function _type($attr,$content) {
		...
		$id = !empty($tag['id'])?$tag['id']:'r'; //定义数据查询的结果存放变量 result
		$parentid =$tag['parentid']; //定义数据查询的结果存放变量 result
		$key    = !empty($tag['key'])?$tag['key']:'n';
		$num = !empty($tag['limit'])?$tag['limit']:'99';//自定义调出栏目数量,可以利用排序把不需要出来的放在后面，仅调出前面需要的几个_by gzse7en
		$field='typeid,name';
		$order='listorder asc';
		$where='parentid = '.$tag['parentid'];
		$sql  = "M(\"Type\")->field(\"{$field}\")->where(\"{$where}\")->order(\"{$order}\")->select();";
			$parsestr  = '';
			$parsestr .= '<?php  $_result='.$sql.'; if ($_result): $'.$key.'=0;';
			$parsestr .= 'foreach($_result as $key=>$'.$id.'):';
			$parsestr .= 'if($'.$key.'<'.$num.') :';
